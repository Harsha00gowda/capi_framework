...
public class DataUtil {
	...
	//This function actually returns two dimensional Object array with only one column and with each row containing one hash table
		...
		public static Object[][] getData(Xls_Reader xls, String module, String testCaseName){
		...
		public static boolean isModuleExecutable(Xls_Reader xls, String moduleName){
            int rows = xls.getRowCount(ProjectConstants.MODULES_SHEET_NAME);
            for(int rNum = 2; rNum <= rows; rNum++){
                String module = xls.getCellData(ProjectConstants.MODULES_SHEET_NAME, "Module", rNum);
                if(module.equals(moduleName)){
                    String runmode = xls.getCellData(ProjectConstants.MODULES_SHEET_NAME, "Runmode", rNum);
                    System.out.println("RUNMODE" + runmode);
                    if(runmode.equalsIgnoreCase("Y"))
                        return true;
                    else
