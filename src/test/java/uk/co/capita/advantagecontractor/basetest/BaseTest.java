...
@Listeners({uk.co.capita.advantagecontractor.util.Listener.class})
...
//				capabilities.setCapability(InternetExplorerDriver.REQUIRE_WINDOW_FOCUS, false);	
		...
		driver.manage().timeouts().pageLoadTimeout(ProjectConstants.PAGE_LOAD_TIMEOUT, TimeUnit.SECONDS);
//		driver.manage().timeouts().implicitlyWait(ProjectConstants.IMPLICIT_WAIT, TimeUnit.SECONDS);
		wait = new WebDriverWait(driver, ProjectConstants.WEBDRIVER_WAIT_TIME);
	...
	//store randomly generated string as contract name in xls_test data sheet
	public void storeContractNameinXls(String contractName){
		xls.setCellData("Repairs and Contract Set Up", "ContractName", 2, contractName);
...
//	@AfterMethod(alwaysRun = true)
    public void endTest(ITestResult result) {
        ...
        if(result.getStatus() == ITestResult.SKIP){
        	test.log(LogStatus.SKIP, "This test is skipped");//Expecting logger
        	reports.endTest(test);
        	reports.flush();
        }
	}
