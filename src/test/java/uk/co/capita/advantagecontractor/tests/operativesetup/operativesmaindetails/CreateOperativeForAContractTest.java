package uk.co.capita.advantagecontractor.tests.operativesetup.operativesmaindetails;
...
import uk.co.capita.advantagecontractor.pages.sessionpages.operativesetup.operativemaindetails.CreateOperativePage;
...
public class CreateOperativeForAContractTest extends BaseTest{
	...
	public String module = "Operatives";

	@Test(dataProvider = "getTestData")
	public void createOperativesForAContract(Hashtable<String, String> table) throws InterruptedException{
...
//verify module run mode
		...
		Object page = launchPage.goToOperativePage(table.get("Browser"));
		//Object page = launchPage.goToOperativePage(table.get("Browser"));

		if (page instanceof LaunchPage){
			reportFailure("Unable to go to Operative page of the AUT");
		}
		test.log(LogStatus.INFO, "Able to go to Operative page of the AUT");
		OperativeMainDetailsListPage operativelistpage = (OperativeMainDetailsListPage)page;

		//Create operative page
		page = operativelistpage.goToCreateOperativePage();
				//.searchAndviewContract(xls.getCellData("Repairs and Contract Set Up", "ContractName", 2));
		if (page instanceof OperativeMainDetailsListPage){
			reportFailure("Unable to go operative page");
		}
		
		test.log(LogStatus.INFO, "Able to and go operative page");
		CreateOperativePage createOperativePage = (CreateOperativePage) page;

		//Create new operative
		createOperativePage.enterOperativeDetailsAndSave(table.get("ClockNumber"));
		if(page instanceof CreateOperativePage){
			reportFailure("Unable to create operative");
		}
		test.log(LogStatus.INFO, "Able to create operative");
