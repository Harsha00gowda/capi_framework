...
public class EditSORTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void editSOR(Hashtable<String, String> table) throws InterruptedException{
		...
		//search for a SOR and click on edit icon
				page = sorListPage.searchAndClickOnEdit(table.get("Description"));
				if (page instanceof SORListPage){
					reportFailure("Unable to search for the SOR and click on edit button");
				}
				
				test.log(LogStatus.INFO, "Able to search for the SOR and click on edit button");
				EditSORPage editSORPage = (EditSORPage)page;
				
				//Editing the SOR
				page = editSORPage.enterSORDetailsAndUpdate(table.get("Description-Edited"));
				if (page instanceof EditPriorityPage){
					reportFailure("Unable to edit the SOR");
				}
				
				test.log(LogStatus.PASS, "Specified SOR was successfully edited");
