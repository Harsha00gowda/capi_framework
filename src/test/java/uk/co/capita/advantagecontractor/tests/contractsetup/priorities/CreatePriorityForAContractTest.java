package uk.co.capita.advantagecontractor.tests.contractsetup.priorities;
...
public class CreatePriorityForAContractTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void createPriorityForAContract(Hashtable<String, String> table) throws InterruptedException{
		...
		//Click on Priorities bar of the contract
		page = contractDetailsMainPage.goToPrioritiesSection();
		if (page instanceof ContractDetailsMainPage){
			reportFailure("Unable to view Priorities section of a contract");
		}
		test.log(LogStatus.INFO, "Able to view Priorities section of a contract after clicking on Priorities bar");
		PrioritiesListPage prioritiesListPage = (PrioritiesListPage)page;

		//Click on Create Priority for the contract
		page = prioritiesListPage.goToCreatePriorityPage();
		if (page instanceof PrioritiesListPage){
			reportFailure("Unable to go to Create Priority page");
		}
		test.log(LogStatus.INFO, "Able to to go to Create Priority page");
		CreatePriorityPage createPriorityPage = (CreatePriorityPage)page;
		
		
		//Enter priority details and save
		page = createPriorityPage.enterPriorityDetailsAndSave(table.get("Priority"), table.get("Description"), table.get("Active Flag"), table.get("Days"), table.get("Hours"), table.get("Job attendance  Flag"));
		if (page instanceof CreatePriorityPage){
			reportFailure("Unable to enter Priority details and save");
		}
