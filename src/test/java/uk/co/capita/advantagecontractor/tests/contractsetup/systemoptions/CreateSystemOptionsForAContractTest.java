...
public class CreateSystemOptionsForAContractTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void createSystemOptionsForAContract(Hashtable<String, String> table) throws InterruptedException{
		...
		//Click on Create System Options button for the contract
		page = systemOptionsListPage.goToCreateSystemOptionsPage();
		if (page instanceof SystemOptionsListPage){
			reportFailure("Unable to go to Create System Options page");
		}
		test.log(LogStatus.INFO, "Able to to go to System Options page");
		CreateSystemOptionsPage createSystemOptionsPage = (CreateSystemOptionsPage)page;
		
		
		//Enter system options details and save
		page = createSystemOptionsPage.enterSystemOptionsDetailsAndSave(table.get("Option"), table.get("Value"));
		if (page instanceof CreateSystemOptionsPage){
			reportFailure("Unable to enter System Option details and save");
		}
