package uk.co.capita.advantagecontractor.tests.contractorsetup.jobcosting;
...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractorsetup.jobcosting.ContractJobCostingPage;
...
public class CreateJobCostingForAContractTest extends BaseTest {
	...
	public void createContractJobNumberForAContract(Hashtable<String, String> table) throws InterruptedException{
		...
		//Click on Contract job costing accordion of the contract
		page = contractDetailsMainPage.goToContractJobCostingAccordion();
		if (page instanceof ContractDetailsMainPage){
			reportFailure("Unable to view Contract job number section of a contract");
		}
		test.log(LogStatus.INFO, "Able to view Contract job costing section of a contract after clicking on contract job costing accordion");
		ContractJobCostingListPage contractJobCostingListPage = (ContractJobCostingListPage)page;	
	
		//Click on Edit contract job costing for the contract
		page = contractJobCostingListPage.goToEditContractCostingPage();
		if (page instanceof ContractJobCostingListPage){
			reportFailure("Unable to go to Edit contract job costing page");
		}
		test.log(LogStatus.INFO, "Able to to go to Contract job number page");
		ContractJobCostingPage contractJobCostingPage = (ContractJobCostingPage)page;
		
		//Enter Contract job costing details and update
		page = contractJobCostingPage.enterContractJobCostingDetailsAndUpdate();
		if (page instanceof ContractJobCostingPage){
			reportFailure("Unable to enter Contract job costing details and update");
		}
		test.log(LogStatus.INFO, "Able to enter Contract job costing details and update");  
