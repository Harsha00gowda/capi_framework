...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.contractorvariance.CreateTradeVariancePage;
import uk.co.capita.advantagecontractor.util.DataUtil;
import uk.co.capita.advantagecontractor.util.ProjectConstants;

public class CreateContractorVarianceForAContractTest extends BaseTest {
	
	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";
	
	@Test(dataProvider = "getTestData")
	public void createContractorVarianceForAContractTest(Hashtable<String, String> table) throws InterruptedException {
		...
		//Click on Contractor Accordion of the contract
		page = contractDetailsMainPage.goToContractorsSection();
		if (page instanceof ContractDetailsMainPage){
			reportFailure("Unable to view Contractors section of a contract");
		}
		test.log(LogStatus.INFO, "Able to view Contractors section of a contract after clicking on Contractors Accordion");
		ContractorVarianceListPage contractorVarianceListpage = (ContractorVarianceListPage)page;
		
		//Click on Create Contract variance for the contract
		page = contractorVarianceListpage.goToCreateTradeVariancePage();
		if (page instanceof ContractorVarianceListPage){
			reportFailure("Unable to go to Create trade variance page");
		}
		test.log(LogStatus.INFO, "Able to to go to Create trade variance page");
		CreateTradeVariancePage createTradeVariancePage = (CreateTradeVariancePage)page;
		
		//Enter Trade variance details and save
		page = createTradeVariancePage.enterTradeVarianceDetailsAndSave();
		if (page instanceof CreateTradeVariancePage){
			reportFailure("Unable to enter Trade variance details and save");
		}
