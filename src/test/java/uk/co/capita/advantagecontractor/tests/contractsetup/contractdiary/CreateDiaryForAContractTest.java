...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.contractdiary.CreateDiaryPage;
...
public class CreateDiaryForAContractTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void createDiaryForAContract(Hashtable<String, String> table) throws InterruptedException{
		...
		if (page instanceof ContractDetailsMainPage){
			reportFailure("Unable to view Diary section of the contract");
		}
		test.log(LogStatus.INFO, "Able to view Diary section of the contract after clicking on Diary bar");
		DiaryListPage diaryListPage = (DiaryListPage)page;

		//Click on Create Diary button for the contract
		page = diaryListPage.goToCreateDiaryPage();
		if (page instanceof DiaryListPage){
			reportFailure("Unable to go to Create Diary page");
		}
		test.log(LogStatus.INFO, "Able to to go to Create Diary page");
		CreateDiaryPage createDiaryPage = (CreateDiaryPage)page;
		
		
		//Enter Diary details and save
		page = createDiaryPage.enterDiaryDetailsAndSave(table.get("Diary Status"), table.get("Diary Text"));
		if (page instanceof CreateDiaryPage){
			reportFailure("Unable to enter Diary details and save");
		}
