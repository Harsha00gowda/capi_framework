...
public class EditSORSkillSetTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void editSkillSet(Hashtable<String, String> table) throws InterruptedException{
		...
		//search and edit the skill set
				page = sorSkillSetListPage.searchSkillSetAndClickOnEdit(table.get("Skill"));
				if (page instanceof SORSkillSetListPage){
					reportFailure("Unable to search for the skill set and click on edit button");
				}
				
				test.log(LogStatus.INFO, "Able to search for the skill set and click on edit button");
				EditCertificatePage editCertificatePage = (EditCertificatePage)page;
				
				//Editing the certificate
				page = editCertificatePage.editCertificatesDetailsAndUpdate(table.get("UpdatedDescription"));
				if (page instanceof EditCertificatePage){
					reportFailure("Unable to edit the certificate");
				}
				
				test.log(LogStatus.PASS, "Specified certificate was successfully edited");
