...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.recharge.EditRechargePage;
...
public class EditRechargeTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void editRecharge(Hashtable<String, String> table) throws InterruptedException{
		...
		//search for a Recharge and click on edit icon
				page = rechargeListPage.searchAndClickOnEdit(table.get("Resource Code"));
				if (page instanceof RechargeListPage){
					reportFailure("Unable to search for the recharge and click on edit button");
				}
				
				test.log(LogStatus.INFO, "Able to search for the recharge and click on edit button");
				EditRechargePage editRechargePage = (EditRechargePage)page;
				
				//Editing the contract
				page = editRechargePage.editRechargeDetailsAndUpdate(table.get("Line No"));
				if (page instanceof EditRechargePage){
					reportFailure("Unable to edit the recharge");
				}
				
				test.log(LogStatus.PASS, "Specified recharge was successfully edited");
