package uk.co.capita.advantagecontractor.tests.contractsetup.contractdiary;
...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.contractdiary.EditDiaryPage;
...
public class EditDiaryTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void editDiary(Hashtable<String, String> table) throws InterruptedException{
		...
		//Click on Diary bar of the contract
		page = contractDetailsMainPage.goToDiarySection();
		if (page instanceof ContractDetailsMainPage){
			reportFailure("Unable to view Diary section of a contract");
		}
		test.log(LogStatus.INFO, "Able to view Diary section of a contract after clicking on Diary bar");
		DiaryListPage diaryListPage = (DiaryListPage)page;
		
		//search for a Diary and click on edit icon
				page = diaryListPage.searchAndClickOnEdit(table.get("Diary Text"));
				if (page instanceof DiaryListPage){
					reportFailure("Unable to search for the diary and click on edit button");
				}
				
				test.log(LogStatus.INFO, "Able to search for the diary and click on edit button");
				EditDiaryPage editDiaryPage = (EditDiaryPage)page;
				
				//Editing the Diary
				page = editDiaryPage.editDiaryDetailsAndUpdate(table.get("Updated Diary Text"));
				if (page instanceof EditDiaryPage){
					reportFailure("Unable to edit the diary");
				}
				
				test.log(LogStatus.PASS, "Specified diary was successfully edited");
