...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.jobtypes.CreateJobTypePage;
...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.systemoptions.CreateSystemOptionsPage;
...
public class CreateJobTypeForAContractTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void createJobTypeForAContract(Hashtable<String, String> table) throws InterruptedException{
		...
		//Click on Create Job Types button for the contract
		page = jobTypesListPage.goToCreateJobTypePage();
		if (page instanceof JobTypesListPage){
			reportFailure("Unable to go to Create Job Types page");
		}
		test.log(LogStatus.INFO, "Able to to go to Job Types page");
		CreateJobTypePage createJobTypePage = (CreateJobTypePage)page;
		
		
		//Enter system options details and save
		page = createJobTypePage.enterJobTypeDetailsAndSave(table.get("Job Type"), table.get("Effective Date"));
		if (page instanceof CreateJobTypePage){
			reportFailure("Unable to enter Job Type details and save");
		}
