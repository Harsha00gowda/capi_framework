package uk.co.capita.advantagecontractor.tests.contractsetup.jobtypes;
...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.jobtypes.EditJobTypePage;
...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.systemoptions.EditSystemOptionsPage;
...
public class EditJobTypeTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void editJobType(Hashtable<String, String> table) throws InterruptedException{
		...
		//Click on Job Types bar of the contract
				page = contractDetailsMainPage.goToJobTypesSection();
				if (page instanceof ContractDetailsMainPage){
					reportFailure("Unable to view Job Types section of a contract");
				}
				test.log(LogStatus.INFO, "Able to view Job Types section of a contract after clicking on Job Types bar");
				JobTypesListPage jobTypesListPage = (JobTypesListPage)page;
		
		//search for a Job Type and click on edit icon
				page = jobTypesListPage.searchAndClickOnEdit(table.get("Job Type"));
				if (page instanceof JobTypesListPage){
					reportFailure("Unable to search for the Job Type and click on edit button");
				}
				
				test.log(LogStatus.INFO, "Able to search for the Job Type and click on edit button");
				EditJobTypePage editJobTypePage = (EditJobTypePage)page;
				
				//Editing the contract
				page = editJobTypePage.editJobTypeDetailsAndUpdate(table.get("On Contract Flag"));
				if (page instanceof EditJobTypePage){
					reportFailure("Unable to edit the Job Type");
				}
				
				test.log(LogStatus.PASS, "Specified Job Type was successfully edited");
