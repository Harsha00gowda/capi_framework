...
import uk.co.capita.advantagecontractor.pages.sessionpages.contractsetup.sorskillset.CreateSORSkillSetPage;
...
public class CreateSORSkillSetForAContractTest extends BaseTest{

	public String testCaseName = this.getClass().getSimpleName(); 
	public String module = "Repairs and Contract Set Up";

	@Test(dataProvider = "getTestData")
	public void createSORSkillSetForAContract(Hashtable<String, String> table) throws InterruptedException{
		...
		//Create skill set for SOR
		page = sorSkillSetListPage.goToCreateSkillSetPage();
		if (page instanceof SORSkillSetListPage){
			reportFailure("Unable to go to Create SOR skill set page");
		}
		test.log(LogStatus.INFO, "Able to to go to Create SOR skill set page");
		CreateSORSkillSetPage createskillset = (CreateSORSkillSetPage)page;

		//Create skill set for SOR
		page = createskillset.enterSkillSetDetailsAndSave(table.get("Skill"));
		if (page instanceof CreateSORSkillSetPage){
			reportFailure("Unable to create skill set details");
		}
		test.log(LogStatus.INFO, "Able to create skill set details");
